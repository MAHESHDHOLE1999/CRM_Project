...
export default function Login() {
  ...
  const onLogin = async (data) => {
    try {
      ...
      toast.error(error.response?.data?.message || t('auth.loginFailed') || 'Login failed');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md animate-slide-in">
        ...
        {/* {submitSuccess && (
          <div className="fixed top-4 right-4 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-xl p-4 flex items-center gap-3 animate-slide-in z-50 shadow-lg">
            ...
            <span className="font-semibold text-green-800 dark:text-green-300">
              {t('auth.successWelcome') || 'Success! Welcome to Ajay Gadhi CRM'}
            </span>
          </div>
        )} */}
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">
            {t('app.title') || 'Ajay Gadhi Bandar'}
          </h1>
          <p className="text-slate-600 dark:text-slate-400">{t('auth.crmPlatform') || 'CRM Platform'}</p>
        </div>
              ...
              <div className="space-y-6 animate-slide-in">
                ...
                <div className="flex items-center justify-between text-sm">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-slate-600 dark:text-slate-400">{t('auth.rememberMe') || 'Remember me'}</span>
                  </label>
                  <button
                    type="button"
                ...
                <div className="text-center">
                  <p className="text-slate-600 dark:text-slate-400 text-sm">
                    {t('auth.newToAjay') || 'New to Ajay Gadhi?'}{' '}
                    <button 
                      type="button"
                      onClick={() => setActiveTab('register')}
                      className="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-semibold transition"
