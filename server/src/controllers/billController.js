...
// File: backend/controllers/billController.js
// UPDATED: Added Registration Date & Time to Bill
...
export const generateBookingBill = async (req, res) => {
  ...
  try {
    ...
    logger.info(`📄 Generating Booking Bill - Booking ID: ${id}`);

    const billNo = booking._id.toString().slice(-6);
    
    // ✅ Registration Date & Time (when booking was registered)
    const registrationDate = new Date(booking.registrationDate);
    const registrationDateStr = `${registrationDate.getDate()}/${registrationDate.getMonth() + 1}/${registrationDate.getFullYear()}`;
    const registrationTimeStr = booking.registrationTime || '10:00';
    
    // ✅ Booking Date & Time (when customer needs items)
    const bookingDate = new Date(booking.bookingDate);
    const bookingDateStr = `${bookingDate.getDate()}/${bookingDate.getMonth() + 1}/${bookingDate.getFullYear()}`;
    const bookingTimeStr = `${booking.startTime || '10:00'} - ${booking.endTime || '18:00'}`;
  ...
  <div class="page">
    ...
    <div class="invoice-header">
      <div class="invoice-info">
        <div class="section-title">${t.invoiceDetails}</div>
        <div class="invoice-number">${t.invoiceNumber}${billNo}</div>
        <div class="info-row"><strong>${t.registrationDate}:</strong> ${registrationDateStr}</div>
        <div class="info-row"><strong>${t.registrationTime}:</strong> ${registrationTimeStr}</div>
        <div class="info-row"><strong>${t.bookingDate}:</strong> ${bookingDateStr}</div>
      </div>
    ...
    <div class="booking-details">
      ...
      <div class="detail-box">
        <div class="detail-label">${t.bookingTime}</div>
        <div class="detail-value">${bookingTimeStr}</div>
      </div>
    </div>
